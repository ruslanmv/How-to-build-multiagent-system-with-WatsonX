...
# Suppress lifecycle warning if you want to ignore it temporarily
...
if not watsonx_api_key or not project_id or not url:
    raise ValueError("Please set the WATSONX_API_KEY, PROJECT_ID, and WATSONX_URL in your .env file.")
...
# Define Agents
# Ensure Agent class is correctly imported from the required library
router = Agent(
    llm=llm,
    role="Router",
...
researcher = Agent(
    ...
    tools=[tavily_tool]  # Use TavilySearchResults directly as the tool
)
...
# Execute the workflow (updated method calls)
def crew_workflow(user_query):
    ...
    if selected_agent == "Researcher":
        ...
        search_result = tavily_tool.invoke(user_query)  # Perform search using TavilySearchResults
        ...
        result = llm.invoke(creator_prompt)  # Use invoke() for the Creator agent
